...
def get_f2fd_pair(vol, bernoulli_mask_ratio=0.5, phase_inversion_ratio=0.1, min_mask_radius=0.05, max_mask_radius=0.1):
    ...
    # Inverse FFT
    ifft_patch1 = torch.fft.irfftn(torch.fft.ifftshift(fft_patch1, dim=(-3, -2)), s=vol.shape)
    ifft_patch2 = torch.fft.irfftn(torch.fft.ifftshift(fft_patch2, dim=(-3, -2)), s=vol.shape)
    
    return ifft_patch1.float(), ifft_patch2.float()

def mask(vol_fft, bernoulli_mask_ratio, phase_inversion_ratio, min_mask_radius, max_mask_radius):
